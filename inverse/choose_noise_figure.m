...
% remove the value field if present
if isfield(inv_model.hyperparameter,'value')
...
function [HP,NF,SE] = HP_for_NF_search(dNF,imdl)
   ...
   hp= search2(dNF,imdl, dx);
    ...
    HP= 10^-hp;
    % calculate NF/SE at chosen hp for returning
...
function hp=search2(dNF, imdl, dx)
   ...
   while isempty(hp) && it < 2
       ...
       % remove non-positive values which cause log10 -> -Inf
       valid = (nf/it) > 0 & ~isnan(nf);
       if sum(valid) < 4
           % not enough valid points for a cubic fit -> leave hp empty
           p = []; hp = [];
       else
           log_nf = log10(nf(valid)/it);
           try
               p = polyfit( dx(valid), log_nf-log10(dNF), 3);
           catch
               p = [];
           end
       ...
       if numel(hp) >1
           ...
           [~, pos] = min(abs(polyval(p2,hp)));
           hp = hp(pos);
       end
   end
   if isempty(hp)
       % fallback: use best available dx based on nf values (robust to zeros/NaN)
       log_nf_all = log10(max(nf/it, eps));
       % set non-finite entries to +Inf so they won't be selected
       log_nf_all(~isfinite(log_nf_all)) = +Inf;
    [~,idx] = min(abs(log_nf_all - log10(dNF)));
    hp = dx(idx);
   end
