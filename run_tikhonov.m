...
inv_template.solve = @inv_solve_core; % 使用 GN 求解器
inv_template.inv_solve_core.max_iterations = 5; % Tikhonov 收敛较快，通常不需要太多迭代
inv_template.inv_solve_core.term_tolerance = 1e-5;
inv_template.inv_solve_core.verbose = 1;

%% 3. Tikhonov 重建
imdl_tik = inv_template;
imdl_tik.hyperparameter.value = hp_tik;
imdl_tik.RtR_prior = @prior_tikhonov; 
if isfield(imdl_tik, 'R_prior'); imdl_tik = rmfield(imdl_tik, 'R_prior'); end
imgr_tik = inv_solve(imdl_tik, vi);

%% 4. 结果展示
hold on;
figure('Name','Tikhonov Reconstruction');
show_slices(imgr_tik);
title('Tikhonov (0th order)');
colorbar;
