...
%% 3. NOSER 重建
imdl_noser = inv_template;
imdl_noser.hyperparameter.value = hp_noser;
imdl_noser.RtR_prior = @prior_noser; 
if isfield(imdl_noser, 'R_prior'); imdl_noser = rmfield(imdl_noser, 'R_prior'); end
imgr_noser = inv_solve(imdl_noser, vi);

%% 4. 结果展示
figure('Name','NOSER Reconstruction');
show_slices(imgr_noser); 
title('NOSER');
colorbar;
