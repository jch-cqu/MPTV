...
function set_defaults(HOMEDIR)
    ...
    eidors_cache('cache_size', 4096^3 );
...
function archdir = set_paths(HOMEDIR, ver,path_array)
    ...
    % Add standard subdirectories if they exist; report missing ones via eidors_msg
    path_list = { ...
        HOMEDIR, ...
        fullfile(HOMEDIR, 'solvers'), ...
        fullfile(HOMEDIR, 'solvers', 'inverse'), ...
        fullfile(HOMEDIR, 'solvers', 'forward'), ...
    fullfile(HOMEDIR, 'solvers', 'forward', 'tools-beta'), ...
        fullfile(HOMEDIR, 'models'), ...
        fullfile(HOMEDIR, 'meshing'), ...
        fullfile(HOMEDIR, 'meshing', 'netgen'), ...
        fullfile(HOMEDIR, 'meshing', 'distmesh'), ...
        fullfile(HOMEDIR, 'meshing', 'gmsh'), ...
    fullfile(HOMEDIR, 'meshing', 'stl'), ...
        fullfile(HOMEDIR, 'tools'), ...
        fullfile(HOMEDIR, 'graphics', 'matlab'), ...
        fullfile(HOMEDIR, 'graphics', 'vtk') ...
        };
    for i = 1:numel(path_list)
        ...
        if exist(p, 'dir')
            ...
            eidors_msg('STARTUP: path not found (skipped): %s', p, 2);
        end
    end
    % external needs recursive add (genpath)
    extp = fullfile(HOMEDIR, 'external');
    if exist(extp, 'dir')
        addpath(genpath(extp));
    end
    % we will be overloading built-in functions. Disable the warning.
    warning off MATLAB:dispatcher:nameConflict
    overloads_dir = fullfile(HOMEDIR, 'overloads');
    if exist(overloads_dir, 'dir')
        addpath(overloads_dir);
    end
    warning on MATLAB:dispatcher:nameConflict

    % Add project-local helpers folder (if present) so custom priors/helpers
    % are available automatically on startup. This mirrors add_helpers_to_path.
    helpers_dir = fullfile(HOMEDIR, 'helpers');
    if exist(helpers_dir, 'dir')
        try
            addpath(genpath(helpers_dir));
            eidors_msg('STARTUP: added helpers folder to path: %s', helpers_dir, 2);
        catch
            eidors_msg('STARTUP: failed to add helpers folder to path: %s', helpers_dir, 1);
        end
    end
...
function print_welcome(HOMEDIR,archdir,ver)
    ...
    eidors_msg('Parameter: cache_size=%.0f MB',eidors_cache('cache_size')/(4096*4096),1);
