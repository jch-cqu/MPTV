...
% 自适应ridge：基于元素到模型边界的距离
% 边界元素（四角）需要更大的ridge
elem_degree = full(sum(A, 2));  % 邻居数
is_boundary = elem_degree < max(elem_degree);  % 边界元素

% 计算到边界的距离
Gfull = graph(A);
boundary_nodes = find(is_boundary);
if ~isempty(boundary_nodes)
    Dmat_bnd = distances(Gfull, boundary_nodes);
    dist_to_boundary = min(Dmat_bnd, [], 1)';
else
    dist_to_boundary = ones(Ne, 1) * 10;
end

% ridge随距离衰减：边界处大，内部小
% 边界(dist=0): ridge_factor = 50
% 内部(dist大): ridge_factor → 1
ridge_factor = 1 + 15 * exp(-dist_to_boundary);  % 更陡峭
adaptive_ridge = opts.ridge * ridge_factor;
Ridge = spdiags(adaptive_ridge, 0, Ne, Ne);
